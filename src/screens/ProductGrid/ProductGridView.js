...
function ProductGridView(props) {
    ...
    return <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        <FlatList
            ...
            data={products}
            numColumns={2}
            renderItem={({ item, index }) => <GridItems
                item={item}
                index={index}
                onPress={() => {
                    let cloneProducts = products.map((eachItem) => {
                        if (item.productName == eachItem.productName) {
                            // return {...eachItem, isSaved: true}
                            return { ...eachItem, isSaved: eachItem.isSaved == false || eachItem.isSaved == undefined ? true : !eachItem.isSaved }
                        }
                        return eachItem
